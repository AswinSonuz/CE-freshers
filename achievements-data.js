/* ==========================================================================
   achievements-data.js
   Edit ONLY this file to update the title and description shown on each
   achievement photo when hovered.

   - "title"  : The bold heading shown on hover  (leave "" for photo-only)
   - "desc"   : The short description below title (leave "" for photo-only)
   ========================================================================== */

const achievementsData = [
  {
    id: "A1",
    title: "",
    desc: "Heartfelt congratulations to the entire team,"
  },
  {
    id: "A2",
    title: "Advika '26",
    desc: "Secured 2nd Position In OPPANA."
  },
  {
    id: "A3",
    title: "Advika '26",
    desc: "Secured 1st Position In Bharathanatyam."
  },
  {
    id: "A4",
    title: "APJAKTU Interzone",
    desc: "Champions at the APJAKTU Interzone Kho-Kho Championship."
  },
  {
    id: "A5",
    title: "",
    desc: ""
  },
  {
    id: "A6",
    title: "Sports Meet",
    desc: "Individual Champion"
  },
  {
    id: "A7",
    title: "Advika '26",
    desc: "Secured 3rd Position In Kollukali."
  },
  {
    id: "A8",
    title: "",
    desc: ""
  },
  {
    id: "A9",
    title: "Annual Sports Meet",
    desc: "Womens Overall & Mens Runner up."
  },
  {
    id: "A10",
    title: "",
    desc: ""
  },
  {
    id: "A11",
    title: "",
    desc: ""
  },
  {
    id: "A12",
    title: "",
    desc: ""
  }
];
